// app/chasm/page.tsx
// Chasm Score Dashboard — GDIOS_SCORE_ENGINE による純粋関数スコア計算
...
function defaultEvidence(): ChasmEvidence {
  return { ...DEMO_SCENARIOS[0].chasmEvidence };
}
...
export default function ChasmScreen() {
  ...
  // デモデータを初期値として自動計算
  const [result,   setResult]   = useState<ChasmScoreResult | null>(() => calculateChasmScore(defaultEvidence()));

  // シナリオ切り替え時に evidence を更新
  useEffect(() => {
    const scenario = DEMO_SCENARIOS.find((s) => s.id === activeScenarioId);
    if (!scenario) return;
    const ev = { ...scenario.chasmEvidence };
    setEvidence(ev);
    setResult(calculateChasmScore(ev));
  }, [activeScenarioId]);
  ...
  const handleSegmentChange = (s: string) => {
    ...
    const ev = scenario ? { ...scenario.chasmEvidence } : defaultEvidence();
    setEvidence(ev);
    setResult(calculateChasmScore(ev));
  };
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr 320px", gap: 24 }}>
          ...
          <div style={{ position: "sticky", top: 0 }}>
            ...
            <div style={{ background: "var(--bg2)", border: "1px solid var(--border)", borderRadius: 10, padding: "20px", marginBottom: 12, position: "relative", overflow: "hidden" }}>
              ...
              <p style={{ fontFamily: "var(--mono)", fontSize: 9, color: "var(--text-tertiary)", textTransform: "uppercase", letterSpacing: "0.08em", marginBottom: 12 }}>{lang === "ja" ? "キャズム突破スコア" : "Chasm Score"}</p>
